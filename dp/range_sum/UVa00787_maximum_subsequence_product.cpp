...
void solve() 
{
    // intuition: Kadanes algorithm but keep track of two numbers, current-negative and current positive.
    // if the overall result is 1, which is netral element of multiplicaiton, then there are no positive elemnts >1 in array -> linear scan and find best single element
    // if elment is 0, restart from 1
    bigint num;
    while(cin>>num){
        vector<bigint> nums;
        while(num != -999999){
        ...
        FOR(i, nums.size()){
            if(nums[i]<0){
                bigint temp = pos;
                pos = max(neg*nums[i], bigint(1ll)); // first negative, positive is impossible afterwards
                neg = temp*nums[i];
            }else if(nums[i] > 0){
                pos = pos*nums[i];
                neg = min(neg*nums[i], bigint(1ll)); // not a neg found yet
            }else{
                neg = bigint(1ll);
                pos = bigint(1ll);
            }
