...
void solve() 
{
    ...
    while(cin >> n){
        ...
        vector<int> LIS(n);
        vector<int> sequenceLengthInc(n, 0);
        int ki=0;
        FOR(i, n){
            int pos = lower_bound(LIS.begin(), LIS.begin() + ki, nums[i]) - LIS.begin();
            LIS[pos] = nums[i];
            sequenceLengthInc[i] = pos;
            if(ki == pos){
                ki++;
            }
        }
        vector<int>LDS(n);
        vector<int> sequenceLengthDec(n, 0);
        int kd= 0;
        FOR(i, n){
            int pos = lower_bound(LDS.begin(), LDS.begin() + kd, nums[n-1-i]) - LDS.begin();
            LDS[pos] = nums[n-1-i];
            sequenceLengthDec[n-1-i] = pos;
            if(kd == pos){
                kd++;
            }
        }
        int ret = 0;
        FOR(i, n){
            int wings = min(sequenceLengthDec[i], sequenceLengthInc[i]);
            ret = max(ret, wings * 2 + 1);
        }
        cout << ret << endl;
    }
