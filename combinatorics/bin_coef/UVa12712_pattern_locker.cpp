...
void solve() 
{
    //intuition: from (L*L C i) but keep track of permutation: (L*L C i) * i! = (L*L)! / (L*L -i)!..
    // Very similar also derivable via first one has L*L second ine (L*L -1) ...
    ll grid, l, u; cin >> grid>>l>>u;
    ll facs = 1;
    ll lauf = grid*grid;
    int i = 0;
    while(i<l){
