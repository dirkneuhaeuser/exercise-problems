...
vector<vector<bigint>> C(201, vector<bigint>(201, 1));
vector<bigint> facs{1,1};
...
void solve() 
{
    // Idea: first of all subtract from the possible white balls all the balls you already know where they will be (1 in between each black at least). For the rest, you can distribute them freely. Formula to put r elements in k bins: (r+k -1) C r
    ll n, s;
    cin >> n >> s;
    ll countBlack = 0;
    FOR(i,s){
    ...
    ll whites = n -countBlack;
    ll boxes = s +1;
    whites -= (s-1); // there has to be at least one!
    if(whites<0) {
        cout << 0;
        return;
    }
    bigint ret = facs[whites+boxes-1]/ (facs[whites] * facs[boxes-1]);
    cout << ret;
