...
vector<bigint> cats = {1}, superCats = {0, 1};
...
int main() 
{
    ...
    for(int i=1; i<=26; ++i){
        cats.push_back(cats.back() * ((i-1)*4 + 2) / ((i-1) + 2));
    }
    for(int i=2; i<=26; ++i){
        superCats.push_back((superCats[i-1] * (6*i - 9)   - superCats[i-2]*(i-3))/i);
    }
...
void solve() 
{
    ...
    // idea: super catalan numbers count the number of trees with n leaves and each inner node has >=2 children
    // there is a bijection from that tree to all possible bracketings (binary and non binary)
    //
    // btw binary bracketing means ops(x1, x2) = ( x1, x2 )..so two elemnts get transformed
    // catalan numbers on the other hand count the binary bracketings, for example: (x(x(xx)))
    ll n;
    while(cin>>n){
        cout << superCats[n] - cats[n-1] <<endl; 
