...
int main() 
{
    ...
    int n = 40;
    for(int i=3; i<=n; ++i){
        dp.push_back(4*dp[i-2] + dp[i-1] + 2*dp[i-3]);
        //cout << dp.back() << endl;
    }
...
void solve() 
{
    // idea: new forms can be created by using last 4 (dp[i-2]) to create L shapes
    // you can use last 2 (dp[i-1]) to add two dots
    // you can use last 6 (dp[i-3]) to use two combined l shapes
    int n;
    cin >> n;
    cout << dp[n];
