...
{
    int m, n;cin >> m >> n;
    // idea: match rows with columns, from source to row capacity:row_sum, from col to sink: col_sum and inbetween add edges between specific col and row for each cell they share
    // Note: This Problem gets now TLE with Maxflow
    // strucutre: source,  m rows, n cols, sink
    MaxFlow mf = MaxFlow(2 + m + n);
    int offset_cols = m+1;
    int sink = n+ m+1;
    ll target = 0;
    FOR(i, m){
        int row_sum; cin >> row_sum;
        target+=row_sum;
        mf.add_edge(0, 1+i, row_sum);

    }
    ll check = 0;
    FOR(j, n){
        int col_sum; cin >> col_sum;
        check += col_sum;
        mf.add_edge(j+offset_cols,sink, col_sum);
    ...
    FOR(i, m){
        FOR(j, n){
            mf.add_edge(1+i,offset_cols + j ,1);
        }
    }
