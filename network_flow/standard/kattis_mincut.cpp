...
void solve() 
{
    int n, m, s, t;
    cin >> n >> m >> s >> t;
    // Note: Residual graph = non-saturated back edges and forward edges of edges that have flow. 
    // To get the left partition of the maxFlow: dfs from source within resiudal-graph (over all non saturated edges) 
    ...
    ll ret = mf.dinic(s, t);
    vector<int> leftNodes = mf.getLeftMinCut(s);
    cout << leftNodes.size() << endl;
    FOR(i, leftNodes.size()){
        cout << leftNodes[i] << endl;
    }
