...
void solve() 
{
    // normal max flow/assignment problem (people to sites)
    int people, sites, site_cap, m; cin >> people >> sites >> site_cap >> m;
    // structure: source, sites, people, sink = 2 + sites + people
    MaxFlow maxFlow = MaxFlow(2 + sites + people);
    int offset_sites = 1;
    int offset_people = sites+1;
    int sink = 2+sites+people -1;
    FOR(i, sites){
        maxFlow.add_edge(0,i+1,site_cap);
    }
    FOR(i, people){
        maxFlow.add_edge(offset_people + i,sink,1);
    }
    FOR(i, m){
        int p, s; cin >> p >> s;
        p--; s--;
        maxFlow.add_edge(s+1, offset_people + p,1);
    }
