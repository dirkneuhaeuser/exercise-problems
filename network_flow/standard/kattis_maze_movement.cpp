...
void solve() 
{
    // normal maxflow. Use gcd for checking the flow capacity
    int n; cin >> n;
    MaxFlow maxFlow = MaxFlow(n+2); // O(V^2 * E)
    ll INF = 1'000'000'001;
    int sink = n+1;
    ...
    FOR(i, n){
        ll num; cin >> num;
        FOR(j, nums.size()){
            int g = __gcd(num, nums[j]);
            if(g > 1){
                maxFlow.add_edge(nums.size()+1,1+j,g, false);
            }
        }
        nums.push_back(num);
    }
    ll minE = 1'000'000'001;
    ll maxE = 1;
    int minI= -1, maxI = -1;
    FOR(i, n){
        if(nums[i]<minE){
    ...
    maxFlow.add_edge(0,minI,INF, false);
    maxFlow.add_edge(maxI,sink,INF, false);
