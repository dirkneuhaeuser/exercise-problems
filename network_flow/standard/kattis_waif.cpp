...
{
    int INF = 1<<30;
    int kids, toys, cats; cin >> kids >> toys >> cats;
    // assignment-Problem with structure: source, kids, toys, category (+1 remaining category), sink;
    
    MaxFlow mf = MaxFlow(2+kids + toys + cats + 1);
    int offset_kids = 1;
    int offset_toys = 1+kids;
    int offset_cats = 1+kids+toys;
    int sink = 2+kids+toys+cats;
    FOR(i, kids){
        mf.add_edge(0, 1+i, 1 );
        int k; cin >> k;
        FOR(j, k){
            int toy; cin >> toy;
            toy--;
            mf.add_edge(1+i, offset_toys+toy ,1);
        }
    }
    vector<bool> used_toys(toys, false);
    FOR(i, cats){
        int k; cin >> k;
        FOR(j, k){
            int toy; cin >> toy;
            toy--;
            used_toys[toy] = true;
            mf.add_edge(offset_toys+toy,offset_cats+i,1);
        }
        int cap; cin >>cap;
        mf.add_edge(offset_cats+i,sink,cap);
    }
    FOR(i, toys){
        if(!used_toys[i]){
            mf.add_edge(offset_toys+i,sink-1,1);
        }
        mf.add_edge(sink-1,sink,INF);
    }
