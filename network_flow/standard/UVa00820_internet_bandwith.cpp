...
private:
    ...
    long long dfs(int cur, int t, long long f=1e18){ // send a single flow from s to t through level graph
...
void solve() 
{
    int nodes, s, t, edges;
    int test=0;
    // standard max flow, sink and target already given. Just compute max flow
    // dinic: O(V^2 * E) = O(10000^2)
    while(cin >> nodes >> s >> t >> edges){
        s--; t--;
        MaxFlow maxFlow = MaxFlow(nodes);
        FOR(i, edges){
            int a, b, bandwith; cin >> a >> b >> bandwith;
            a--; b--;
            maxFlow.add_edge(a, b, bandwith, false);
        }

        long long max_flow = maxFlow.dinic(s, t);
