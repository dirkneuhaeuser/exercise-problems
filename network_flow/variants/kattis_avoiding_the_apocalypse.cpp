...
void solve() 
{
    // max flow with time. In order to also reflect the time constraint, create (s+1) nodes for each location at each given time stamp. When passing a road, then we pass to the specific target node
    // in the future.
    int INF = 1 << 30;
    int n; cin >> n;
    int init, g, s; cin >> init >> g >> s;
    // structure: source, n locations a (s+1) time stamps, sink
    MaxFlow mf = MaxFlow(2 + n*(s+1));
    int sink = 1 + n*(s+1); 

    mf.add_edge(0, (init-1)*(s+1) + 1, g);
    int m; cin >> m;
    FOR(i, m){
        int med; cin >> med;
        FOR(times, s+1){
            mf.add_edge(1 + (med-1)*(s+1) + times, sink, INF);
        }
    }
    int r; cin >> r;
    FOR(i, r){
        ...
        FOR(times, s+1){
            if(times + t > s) break;
            mf.add_edge(1 + (a-1)*(s+1) + times, 1 + (b-1)*(s+1) + times + t, p);
        }

    }

    // waiting at current locatin is allowed
    FOR(i, n){
        FOR(times, s){
            mf.add_edge(1 + i*(s+1) + times, 1 + i*(s+1) + times + 1, INF);
        }
    }
