...
void solve() 
{
    // Idea: Maxflow problem with time and modulo. Blow up each vertix k times (because we are interested in an always moving window of k, if a solution includes that product gets delivered at k+1, then it might be crashing with another product). Thus at each edge, deliver a product from t to (t+1) %k
    int n, k, m; cin >> n >> k >> m;
    MaxFlow mf(n*k+2);
    int sink = n*k + 1;
    FOR(j, m){
        ...
        FOR(t, k){
            int goalTime = (t+1)%k;
            mf.add_edge(offset_a + t, offset_b + goalTime, 1);
        }
    }
    // connet last junction with the sink (warehouse)
    int offset_n = (n-1)*k + 1;
    FOR(t, k){
        mf.add_edge(offset_n + t, sink, INF);
    }
    // connect all beginning k junctions with source
    FOR(i, k){
        int offset_k = i*k + 1;
        mf.add_edge(0,offset_k + i,1); // + i because of the different time
