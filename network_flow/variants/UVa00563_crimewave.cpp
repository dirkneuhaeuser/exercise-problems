...
void solve() 
{
    int n, m; cin >> n >> m;
    int banks; cin >> banks;
    int totalNodes = (n+2) * (m+2); // plus 2 because we add a 'frame' around the matrix, which serves as 'out of town'

    // As nodes can only be used once, we are looking for independent paths -> each vertex has a capacity of 1
    // structure: source, 2*totalNodes, sink.  Wee need 2*totalNodes, because we add vertex capacity of 1.
    // We connect all banks to be robbed to the source, and all frame-out of city nodes to the sink.
    // Note, Vertex capacity of 1 also implise edge capacity of 1, so we can actually mess with their capacity, I set it to INF, but 1 would also be ok.
    MaxFlow mf = MaxFlow(2+2*totalNodes);
    int offsetNodesIn = 1;
    int offsetNodesOut = 1 + totalNodes;
    ...
    for(int i=0; i<n+2; ++i){
        for(int j=0; j<m+2; ++j){
            ...
            // vertex restriction
            mf.add_edge(curNode + 1, curNode + offsetNodesOut, 1);

            if(i==0 || j == 0 || i == n+1 || j == m+1) continue;

            // each output node is connected to the input neighbour nodes
            mf.add_edge(curNode + offsetNodesOut, curNode + 1 + 1, INF);
            mf.add_edge(curNode + offsetNodesOut, curNode + 1 - 1, INF);
            mf.add_edge(curNode + offsetNodesOut, curNode + 1 + m+2, INF);
            mf.add_edge(curNode + offsetNodesOut, curNode + 1 - (m+2), INF);
        }
    }
    vector<int> nodes;
    FOR(i, banks){
        ...
        mf.add_edge(0, nodeIn + 1, INF);
    }
    FOR(i, n+2){
        int l = i*(m+2) + offsetNodesOut;

        int r = i*(m+2) + (m+1) + offsetNodesOut;
        mf.add_edge(l, sink, INF); // left frame
        mf.add_edge(r, sink, INF); // right frame
    }
    for(int j=1; j<m+1; ++j){
        int t = j + offsetNodesOut;
        int b = j + (n+1)*(m+2) + offsetNodesOut;
        mf.add_edge(t, sink, INF); //top frame
        mf.add_edge(b, sink, INF); //bottom frame
    }
