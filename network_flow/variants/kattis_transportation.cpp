...
void solve() 
{
    // idea: very basic assignment problem. Assign transport companies to delivery routes. As they can only deliver from at most one state to another, use vertex capcacity.
    int s,r,f,t; cin >> s >> r >> f >> t;
    vector<int> suppliers, factories;
    ...
    FOR(i, r){
        ...
        suppliers.push_back(state2Idx[state]);
    }
    FOR(i, f){
        string state; cin >> state;
        if(!state2Idx.count(state)){
        ...
        factories.push_back(state2Idx[state]);
    }
    vector<vector<int>> trans(t, vector<int>());
    FOR(i, t){
        int num; cin >> num;
        FOR(j, num){
            string state; cin >> state;
            ...
            trans[i].push_back(state2Idx[state]);
        }
    }

    // structure: source, s states, t transportation firms + vertex capacity, sink
    // difficulty: strings to numbers
    MaxFlow mf = MaxFlow(s+2*t + 2);
    int offsetTransIn = s+1;
    int offsetTransOut = t+s+1;
    int sink = s+2*t + 1;

    FOR(i, r){
        mf.add_edge(0, suppliers[i] + 1, 1);
    }
    FOR(i, f){
        mf.add_edge(factories[i] + 1, sink, 1);
    }
    FOR(i, t){
        // each transport company can only deliver once
        mf.add_edge(i+offsetTransIn, i +offsetTransOut, 1);

        FOR(j, trans[i].size()){
            // trnsport company can deliver from and towards other states
            mf.add_edge(trans[i][j] + 1, i +offsetTransIn, 1);
            mf.add_edge(i +offsetTransOut, trans[i][j] + 1, 1);
        }
    }
