...
void solve() 
{
    // idea: The most difficult max flow problem for me. Connect all down-fields with source and all up fields with sink, both with capacity B.
    // Now connect each neighboud element with capacity A. Each cell can then cause only B trouble and has the task to give the reamining flow to its neighbours.
    // Two cases for higher fields (WLOG also for lower):
    // 1. Case: A higher field has less input flow than B, then transfer all of it to the sink (no changes)
    // 2. Case: A higher field has more input flow than B, then the field takes B and allocates the remaining toward its neighbours. Lets assume we only have one high neighbour.
    //       case 2.1 Remaining to this high neighbour is more than A: Send only A. The equivalent is, that the current high field will be changed to low field.
    //       case 2.2 Remining to this neighbour is less than A: Send less than A. The equivalent is, that the current high field stays high
    //                field (Its not worth it, as changing to a lower field will not get rid of the border between the now low and the still high neighbour field.
    int n, m, a, b;
    cin >> n >> m >> a >>b;
    MaxFlow mf(n*m+2);
    int sink = n*m +1;
    FOR(i, n){
        FOR(j, m){
            int curCell = i*m+j;
            if(i<n-1){
                mf.add_edge(curCell+1, curCell+m + 1, a, false);
            }
            if(j<m-1){
                mf.add_edge(curCell+1, curCell+1 + 1, a, false);
            }
            char cell; cin >> cell;
            if(cell == '.'){
                mf.add_edge(0, curCell+1, b);
            }
            if(cell == '#'){
                mf.add_edge(curCell+1,sink, b);
            }
        }
    }
