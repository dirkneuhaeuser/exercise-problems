...
void solve() 
{
    // intuition: Basic min-cut problem, add a frame (outter world) and add vertex capacity to each node.
    int n, m, c; cin >> n >> m >> c;
    ...
    n+=2; m+=2;
    // strucutre: grid + frame and each two times because of vertex capacity
    MaxFlow mf(m*n*2+1);
    int offset_vc = m*n;
    int sink = m*n*2;
    int idx;
    FOR(i, m){
        FOR(j, n){
            ...
            if(i==0 || j == 0 || i == m-1 || j == n-1){
                // connect frame with sink (out of town)
                mf.add_edge(offset_vc + curNode,sink,INF);
                costOfVertex =  INF;
            }else{
                char cur = grid[i-1][j-1];
                if(cur == 'B' || cur =='.'){
                    if(cur == 'B') idx = curNode;
                    costOfVertex = INF;
                }else{
                    costOfVertex = cost[(int)(cur - 'a')];
                }
            }
            // vertex capacity of each node
            mf.add_edge(curNode, offset_vc + curNode, costOfVertex);

            if(i > 0){
                mf.add_edge(offset_vc + curNode, curNode -n, costOfVertex);
            }
            if(i < m-1){
                mf.add_edge(offset_vc + curNode, curNode +n, costOfVertex);
            }
            if(j > 0){
                mf.add_edge(offset_vc + curNode, curNode -1, costOfVertex);
            }
            if(j < n-1){
                mf.add_edge(offset_vc + curNode, curNode +1, costOfVertex);
            }
