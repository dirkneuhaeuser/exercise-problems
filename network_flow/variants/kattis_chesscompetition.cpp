...
void solve() 
{
    // Intuition: Problem seems greedy, but it isnt. However you can greedily check if a team can even reach the max point of the current winning team.
    // Idea:
    // For each team, let it win all opcoming matches and for the remaining matches do a max-flow: source, teams, matches, sink
    // where the edge from source to the other teams has the capacity of the difference towards your highscore. (they can not make more points than you in total)
    // then each team is connected to its opcoming matches (with capacity 2).
    // If the maxflow can assign all points from all remaining matches, then you can win still
    ...
    FOR(i, n){
        int matchesForI = 0;
        FOR(k, matches.size()){
            if(matches[k].first == i || matches[k].second == i) matchesForI++;
        }
        int bestOutcome = matchesForI * 2 + p[i];
        if(bestOutcome < maxE){
            continue;
        }
        int matchesToModel = matches.size() - matchesForI;


        //structure: source, n teams, matchesToModel matches, sink
        MaxFlow mf(2+n+matchesToModel);
        int matchesOffset = n+1;
        int sink = 1+n+matchesToModel;
        FOR(j, n){
            if(j==i) continue;
            mf.add_edge(0, j+1, bestOutcome-p[j]);
        }
        int idxMatches = 0;

        FOR(j, matches.size()){
            if(matches[j].first == i || matches[j].second == i) continue;
            mf.add_edge(matches[j].first+1, idxMatches+matchesOffset, 2);
            mf.add_edge(matches[j].second+1, idxMatches+matchesOffset, 2);
            mf.add_edge(idxMatches+matchesOffset,sink, 2);
            idxMatches++;
        }
        ll flow = mf.dinic(0, sink);
        ll target = matchesToModel * 2;
        if(flow == target){
            cout << i+1 << " ";
        }
