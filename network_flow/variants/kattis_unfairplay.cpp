...
{
    int n, m;
    // idea: normal check if you team can win. MaxFlow problem, assign each team its max. points in order to still lose and check if flow equals the total match points.
    // Then, a simple inside view into the used edges of each match will tell us, the winner or loser (Assignmentproblem with restriciton).
    bool start = true;
    while(cin >> n >> m){
        if(!start) cout << endl;
        start = false;
        ...
        if(abort) continue;
        // structure: source, n-1 teams, m matches (ours are there, but not used), sink
        MaxFlow mf(1+n+m);
        int offsetMatches = n;
        int sink = n+m;
        FOR(i, n-1){
            int maxPoints = points.back() - points[i] - 1;
            mf.add_edge(0, i+1, maxPoints);
        }
        FOR(j, m){
            int a = matches[j].first;
            int b = matches[j].second;
            if(a==n-1 || b == n-1) continue;
            mf.add_edge(a+1, j+offsetMatches, 2);
            mf.add_edge(b+1, j+offsetMatches, 2);
            mf.add_edge(j+offsetMatches, sink, 2);
        }
        ll flow = mf.dinic(0, sink);
        if(flow  != realMatches<<1){
            cout << "NO" << endl;
            continue;
        }
        FOR(j, m){
            ...
            auto e = mf.get_used_backward_edges(j+offsetMatches);
            if(e.size()==2){
                cout << "1 ";
            }else{
                int teamIdx = get<0>(e[0]);
                if(teamIdx == a+1){
                    cout << "0 ";
                }else{
                    cout << "2 ";
