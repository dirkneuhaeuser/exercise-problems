...
void solve() 
{
    // Complicated min cut problem. Idea: PLACE BOTTLENECKS OF BOTTOM AND TOP COMPONENTS ON OPPOSIDE SIDES:
    // bottom elements have bottleneck towards the sorce and have INF capacity towards the sink. For Top elements vice versa.
    // This set up alone will give us the cost for placing the elements when not considering the interconnection cost.
    // Now an interconnection cost r can increase the min-cut(overall cost), iff connected with top and bottom elements:
    // this way, we can use the INF part of either parts and thus increase by r. However, when the interconnection cost is in-between two components of the same side,
    // then, the bottleneck will prevent an increase.
    int n, m; cin >> n >> m;
    int INF = 1 <<30;
    vector<int> t, b, c;
    FOR(i, n){
    ...
    MaxFlow mf = MaxFlow(2+n);
    int sink = n+1;
    FOR(i, n){
        if(c[i]==0){// both allowed
            mf.add_edge(i+1, sink, t[i]);
            mf.add_edge(0, i+1, b[i]);
        }else if(c[i] == -1){ // bottom
            mf.add_edge(i+1, sink, INF);
            mf.add_edge(0, i+1, b[i]);
        }else if(c[i] == 1){ // top
            mf.add_edge(i+1, sink, t[i]);
            mf.add_edge(0, i+1, INF);
        }
    }
    FOR(i, m){
        int a, b, r; cin >> a >> b >> r;
        mf.add_edge(a,b,r,false);
    }
