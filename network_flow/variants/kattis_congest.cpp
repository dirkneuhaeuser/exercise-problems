...
private:
    ...
    bool bfs(int s, int t){ // construct level graph and check if more flow is possible
        ...
        while (!q.empty()){
            ...
            for(int idx: graph[cur]){
                auto &v = get<0>(edges[idx]);
                auto &cap = get<1>(edges[idx]);
                auto &flow = get<2>(edges[idx]);
    ...
    long long dfs(int cur, int t, long long f=1e18){ // sends a single flow from s to t through level graph O(E)
        ...
        for(int &i = last[cur]; i<(int)graph[cur].size(); ++i){
            ...
            auto test = edges[graph[cur][i]];
            auto &v = get<0>(edges[graph[cur][i]]);
            auto &cap = get<1>(edges[graph[cur][i]]);
            auto &flow = get<2>(edges[graph[cur][i]]);
...
{
    // downtown at 1
    // idea: Firstlt get rid of all edges which are not otpmial, which means, they would lead to a detour.
    // Then create a max flow problem. Each each has a capacity of 1. To deal with the fact, that a road could be used more than once at different times,
    // we repeat the mf for each possible dinstance towards the downtown.
    int n, m, c; cin >> n >> m >> c;
    ...
    vector<int> disToDowntown = dijkstra(0, graph);
    
    // create reduced graph, in which there are only optimal edges
    vector<vector<int>> newGraph(n, vector<int>());
    FOR(i, n){
        FOR(j, graph[i].size()){
            int goal = graph[i][j].first;
            if(i == goal) continue;
            if(disToDowntown[i] < disToDowntown[goal]) continue;
            if(disToDowntown[i] - graph[i][j].second == disToDowntown[goal]){
                newGraph[i].push_back(goal);
            }
        }
    }
    set<int> distances (disToDowntown.begin(), disToDowntown.end());
    int ret = 0;
    for(int d: distances){
        // now repeat maxflow for all possible distances, because these dudes can never share the same edge, similtanously apply edge weight of 1.
        //structure: 1 source, n nodes. Sink not needes = downtown
        bool toCheck = false;
        FOR(i, n){
            if(disToDowntown[i] == d && commuters[i]){
                toCheck = true;
            }
        }
        if(!toCheck) continue; // no commuters start from this distance
        if(d == INF) continue; // the commuter can't reach downtown
        MaxFlow mf(n+2);
        int sink = n+1;
        FOR(i, newGraph.size()){
            FOR(j,newGraph[i].size()){
                mf.add_edge(i+1, newGraph[i][j] +1, 1);
            }
            if(disToDowntown[i] == d){
                mf.add_edge(0, i+1, commuters[i]); // source to commuter nodes
            }
        }
        mf.add_edge(1, sink, INF);
        ret += mf.dinic(0, sink);
