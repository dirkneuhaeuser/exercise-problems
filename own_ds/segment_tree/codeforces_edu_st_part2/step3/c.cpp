...
private:
    ...
    ll query(ll p, ll L, ll R, ll i, ll j, ll val, ll minL){ // O(logn)
        propagate(p, L, R);
        if(L > j || R < i) return -1;
        if(st[p] < val) return -1;
        if(R < minL) return -1;
        if(L==R && st[p] >= val && L >= minL){ // with == would be sufficent
            return L;
        }
        ll m = (L+R)/2;
        ll ans1 = query(l(p), L, m, i, j, val, minL);
        if(ans1 != -1) return ans1;
        ll ans2 = query(r(p), m+1, R, i, j, val, minL);
        return ans2;
    }
public:
    ST(ll size) : n(size) {
    ...
    ll query(ll i, ll j, ll val, ll minL){
        // queries [i,..j]
        return query(1, 0, n-1, i, j, val, minL);
    }
};
...
void solve() 
{
    ...
    // about: Addition + first element bigger than val, which index > minL
    int n, q;
    scanf("%d %d", &n, &q);
    vi init(n, 0);
    ST st(init);
