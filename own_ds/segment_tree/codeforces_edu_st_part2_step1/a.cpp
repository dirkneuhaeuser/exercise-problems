...
void solve() 
{
    ...
    // about: We habe range add updates, but only point queries, so we don't need to recalculate the sum for each inner node and can simply use lazy propagations to memorise the operations for each subtree, to eventually if queried also apply them.
    int n, q; //cin >> n >> q;
    scanf("%d %d", &n, &q);
    vi init(n, 0);
    SegmentTree st(init);
