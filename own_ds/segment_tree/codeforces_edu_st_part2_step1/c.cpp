...
void solve() 
{
    ...
    // about: Assign values to segments and have a point query. Because of the point query, we don't need to worry about calculating intermediate results, but only on propagating the applied function when needed. Here we need to particularly take into consideration to MERGE (here: overwrite them)the operation of the propagted mother with its childern.
    int n, q; //cin >> n >> q;
    scanf("%d %d", &n, &q);
    vi init(n, 0);
    SegmentTree st(init);
