...
private:
    ...
    ll query(ll p, ll L, ll R, ll val, int lmin){ // O(logn)
        propagate(p, L, R);
        ...
        ll m = (L+R)/2;
        ll ans1 = query(l(p), L, m, val, lmin);
        if(ans1 != -1) return ans1;
        ll ans2 = query(r(p), m+1, R, val, lmin);
        return ans2;
    }
public:
    ...
    ll query(ll val, int lmin){
        // queries [i,..j]
        return query(1, 0, n-1, val, lmin);
    }
};
// ST with Nodes
//vector<Node> arr;
...
void solve() 
{
    ...
    // about: Use a segment-tree with max opeartions and change query function s.t. it goes down the path only if max value is greater than the given value x and also the R idx of the segment is greater than given lmin. As in one segment which is divided by lmin, you don't know upfront if the maxvalue can be found left or right of lmin, there is a chance to have an overhead of log n and go wrong,
    int n, q; cin >> n >> q;
    vector<Node> nums(n);
    FOR(i, n){
        cin >> nums[i].val;
    ...
    FOR(i, q){
        ...
        if(type == 1){
            ...
            nums[idx].val = val;
            st.update(idx, idx, nums[idx]);
        }else{
            int x, lmin; cin >> x >> lmin;
            // there only O(log n) segments which are divided by lmin (underneath in tree), that means in worst case, we go down the tree once to the wrong a[i]>=x and have an additional 
            // overhad of O(log n). Thus: overall complexity remains O(log n)
            cout << st.query(x, lmin) << endl;
        }
    }
