...
void solve() 
{
    ...
    // about: Range Update, Point Query with basic BIT technic. Wenn adding val to [i, .. j], we add val to j and subtract it from i-1. When querying for index i, we calc the sum from [i,.. n]
    int n, q; //cin >> n >> q;
    scanf("%d %d", &n, &q);
    vi nums(n+1, 0);
    SegmentTree st(nums);
    FOR(i, q){
        ...
        if(type == 1){
            int l, r, v;// cin >> l >> r >> v;
            scanf(" %d %d %d", &l, &r, &v);
            // we have an additional 0 element, so we need to add +1 to our idx but here: we only query until r-1, so r is ok, end also we need to subtract l not l-1, so l again ok
            nums[r]+=v;
            nums[l]-=v;
            st.update(r, r, nums[r]); 
            st.update(l, l, nums[l]);
        }else{
            int idx;// cin >> idx;
            scanf(" %d", &idx);
            idx++;
            ll ans = st.query(idx, n);
            printf("%lld\n", (ll)ans);
            //cout << ans << endl;
        }
    }
