...
private:
    ...
    int query(ll p, ll L, ll R, int &k){ // O(logn)
        //cout << L << " " << R << " " << st[p].val << " " << k << endl;
        if(k == 0)return -1;
        propagate(p, L, R);
        if(st[p].val < k){
            k-=st[p].val;
        ...
        ll m = (L+R)/2;
        int ans1= query(r(p), m+1, R, k);
        if(ans1 != -1) return ans1;
        int ans2= query(l(p), L, m, k);
        return ans2;
    }
public:
    ...
    int query(int elementsBigger){
        // queries [i,..j]
        int target = elementsBigger + 1; // bigger + equal
...
void solve() 
{
    ...
    // about: In Inversion1, we set the all values to 0 in our ST and then we went from left to right of the permutation, asking our tree for how many bigger elements we alread have and then setting this very number to 1 as well. This probleme is reversed, here all numbers are already set to 1, then we go from right to left and asking for the x+1 number already set from the right in the ST (just reversed).
    ...
    vi ret;
    FOR(i, n){
        int idx = st.query(nums[n-1-i]);
        nodes[idx].val = 0;
        st.update(idx, idx, nodes[idx]);
        ret.push_back(idx+1);
    }
    FOR(i, n){
        cout << ret[n-1-i] << " ";
