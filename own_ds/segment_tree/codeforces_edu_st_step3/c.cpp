...
void solve() 
{
    ...
    // about: advanced st/bit modelling. To get all the segments inside a segments, set all left numbers to 1 and all rights to 0. Then go from right to left in our array and cnt the number of 1 in that segment(area). after proccesing this right number update the left to be 0;
    int n; cin >> n;
    vi left(n, -1), nums(n*2), init(n*2, 0);
    FOR(i, n*2){
        int num; cin >> num;
        num--;
        if(left[num] == -1) {
    ...
    for(int i=n*2-1; i>=0; --i){
        ...
        ret[nums[i]] = st.query(left[nums[i]], i) - 1;
        st.update(left[nums[i]], left[nums[i]], 0); // right cant be part of a segment no more -> clear left
    }
    FOR(i, n){
        cout << ret[i]<< " ";
    }
