...
private:
    ...
    int query(ll p, ll L, ll R, int &k){ // O(logn)
        ...
        ll m = (L+R)/2;
        return query(l(p), L, m, k) + query(r(p), m+1, R, k);
        //cout << L << " " << R << " " << k << " " << ans <<endl;
        //return ans;
    }
public:
...
void solve() 
{
    ...
    // about: Find the kth order, here we use a st tree. Limitation of small array is met
    int n, m; cin >> n >> m;
    vector<Node> nums(n);
    FOR(i, n){
        int num;
    ...
    FOR(i, m){
        int a, b; cin >> a >> b;
        if(a == 1){
            nums[b].val = nums[b].val^1;
            st.update(b, b, nums[b]);
        }else{
