...
private:
    ...
    ll query(ll p, ll L, ll R, ll val){ // O(logn)
        propagate(p, L, R);
        ...
        ll m = (L+R)/2;
        ll ans1 = query(l(p), L, m, val);
        if(ans1 != -1) return ans1;
        ll ans2 = query(r(p), m+1, R, val);
        return ans2;
    }
public:
...
void solve() 
{
    ...
    // about: Use a segment-tree with max opeartions and change query function s.t. it find the smallest integer which has a value greater than a given value
    int n, q; cin >> n >> q;
    vector<Node> nums(n);
    FOR(i, n){
        cin >> nums[i].val;
