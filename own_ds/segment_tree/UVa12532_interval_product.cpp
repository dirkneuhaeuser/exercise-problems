...
void solve() 
{
    // problem: https://onlinejudge.org/index.php?option=com_onlinejudge&Itemid=8&page=show_problem&problem=3977
    // about: Segment-Tree problem. Range product queries with point updates. As Product has no easy inverse (dividing by 0 is ugly in BIT to reduce current segment), we rather use segment-trees here. Adv: we only multiply segments which are actually used. Further note that we have to reduce all numbers to -1, 0 and 1 as otherwise the product will explode.
    int n, q;
    while(cin >> n >> q){
        vi nums(n);
        FOR(i, n){
