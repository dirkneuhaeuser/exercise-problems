...
private:
    ...
    ll conqUpdate(ll prev, ll op, ll len){
        if(NEUTRAL_UPDATE == op){
            return prev;
        }
        return len*op;
    }
    ...
    void propagate(ll p, ll L, ll R){ // propagate the lazy flag one level further down
        st[p] = conqUpdate(st[p], lazy[p], R-L+1);
        if(L!=R){
            lazy[l(p)] = conqUpdate(lazy[l(p)], lazy[p], 1);
            lazy[r(p)] = conqUpdate(lazy[r(p)], lazy[p], 1);
        }
    ...
    void update(ll p, ll L, ll R, ll i, ll j, ll val){ // O(logn)
        ...
        if(L>=i && R<=j){  // goal: narrow L and R down, s.t. the intervall of this node captured. == would be sufficient
            st[p] = conqUpdate(st[p], val, R-L+1);
            if(L != R){
                lazy[l(p)] = conqUpdate(lazy[l(p)], val, 1);
                lazy[r(p)] = conqUpdate(lazy[r(p)], val, 1);
            }
        } else{
            ll m = (L+R)/2;
...
void solve() 
{
    ...
    // about: Basic ST but here Assignment is not distributive wrt to sum. So we need to be smart when using lazy propagation. Also, assigment doesn't know a fixed neutral element.
    int n, q;
    scanf("%d %d", &n, &q);
    vi init(n, 0);
    ST st(init);
