...
private:
    ...
    void update(ll p, ll L, ll R, ll i, ll j, ll val){ // O(logn)
        ...
        if(L>=i && R<=j){  // goal: narrow L and R down, s.t. the intervall of this node captured. == would be sufficient
            st[p] += val; // when flags are intercontected: better to set kids lazy flag here
            if(L != R){
                lazy[l(p)] += val;
                lazy[r(p)] += val;
            }
...
{   
    ...
    // about: Basic ST, Addition is distributive wrt to minimum -> normal lazy propagation
    int n, q;
    scanf("%d %d", &n, &q);
    vi init(n, 0);
    SegmentTree st(init);
