...
vector<ll> f(vector<ll> x){
    ll top = 1;
    vector<ll> ret;
    for(int i = 0; i<x.size(); ++i){
        ll bot = i == (x.size() -1)? 1: x[i+1];
        ret.push_back((top*bot + 1)/ x[i]);
        top = ret.back();
    }
    return ret;
}
...
void solve() 
{
    ...
    while(cin >>n &&n){
        ...
        // idea: frieze pattern will repeat very quickly -> do floyd's cycle detection on vectors


        vector<ll> t = f(arr), h = f(f(arr));
        while(t!=h){
            t = f(t); h = f(f(h));
        }
        t = arr;
