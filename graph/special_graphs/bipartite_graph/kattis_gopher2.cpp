...
ld dis(pdd a, pdd b){
    return powl(powl(abs(a.first - b.first), 2) + powl(abs(a.second - b.second), 2), 0.5);
}
void solve() 
{
    // problem: https://open.kattis.com/problems/gopher2
    // about: Basic MCBM problem. Here Dinic runs in O(sqrt(n+m) * n*m), because the graph is bipartite
    int n, m, s, v; 
    while(cin >> n >> m >> s >> v){ 
        vector<pdd> gopher, holes;
        FOR(i, n){
            ld x, y; cin >> x >> y;
        ...
        // structure: source, n gopher, m holes, sink
        MaxFlow mf(n+m+2);
        int offset_holes = 1+n;
        int sink = n+m+1;
        FOR(i, n){
            mf.add_edge(0, i+1, 1);
        }
        FOR(j, m){
            mf.add_edge(offset_holes+j, sink, 1);
        }
        for(int i=0; i<gopher.size(); ++i){
            ...
            for(int j=0; j<holes.size(); ++j){
                pdd h = holes[j];
                ld d = dis(r, h);
                if(d/v <= (ld)s+0.000001){
                    mf.add_edge(1+i, offset_holes+j, 1);
                }
            }
        }
