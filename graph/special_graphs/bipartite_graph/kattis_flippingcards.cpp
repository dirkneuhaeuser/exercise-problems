...
void solve() 
{
    // problem: https://open.kattis.com/problems/flippingcards 
    // about: Hidden MCBM problem. Left side use the cards and on the right side the possible values. It is possible if maxflow == n.
    // complexity: As bipartite O(sqrt(n+maxV) * n*2) = O(sqrt(50K + 2*50K) * 2*50K) = O(sqrt(150K) * 100K);
    int n; cin >> n;
    vector<pii> cards;
    int maxV = 0;
    FOR(i, n){
    ...
    // bipartite structure: source, n cards, possible values, sink
    MaxFlow mf(n + maxV + 2);
    int offset_values = n+1;
    int sink = n + maxV + 1;
    FOR(i, n){
        mf.add_edge(0, 1+i, 1);
        mf.add_edge(i+1, offset_values + cards[i].first-1, 1);
        mf.add_edge(i+1, offset_values + cards[i].second-1, 1);
    }
    FOR(j, maxV+1){
        mf.add_edge(offset_values + j, sink, 1);
    }
    ll ans = mf.dinic(0, sink);
    if(ans == n){
        cout << "possible";
    }else{
        cout << "impossible";
