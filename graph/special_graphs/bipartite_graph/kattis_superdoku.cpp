...
void solve() 
{
    ...
    // problem: https://open.kattis.com/problems/superdoku
    // about: Advanced Matching problem. First observation is that, each valid prefilled matrix can be filled completely to get an overall valid matrix. This can be proven by the Hall's Mariage Theorem. After this check, we will fill the missing rows, row by row. Let i be the current row, we will match the positions of this row, to the n numbers. Here we only have edges between a certain column of this row and a number, if that number hasent apeared yet in that column.
    //
    int n, k; cin >> n >> k;
    ...
    FOR(i, k){
        ...
        if(row.size() != n) ok = false;
    }
    FOR(j, n){
        if(cols[j].size() != k) ok = false;
    }
    if(!ok){
        cout << "no";
        return;
    ...
    cout << "yes" << endl;
    FOR(i,n-k){
        // this row to n numbers
        // structure: source, n elements of this row, n distinct number (1,..n), sink
        MaxFlow mf(n+n+2);
        int offset_num = n+1;
        int sink = n+n+1;
        FOR(j, n){
            mf.add_edge(0, 1+j, 1);
            // get all numbers for the j column
            FOR(l, n){
                if(!cols[j].count(l)){ // O(n^3 log(n))
                    mf.add_edge(1+j, offset_num+ l,1);
                }
            }
        }
        FOR(l, n){
            mf.add_edge(offset_num+l, sink, 1);
        }
        mf.dinic(0, sink); // O(sqrt(n+n) * 2n);
        FOR(j, n){
            auto used = mf.get_used_forward_edges(j+1);
            int target = get<0>(used[0]);
            int targetIdx = target - offset_num;
            cols[j].insert(targetIdx);
            nums[i+k][j] = targetIdx+1;
        }
    }
    FOR(i, n){
        FOR(j, n){
