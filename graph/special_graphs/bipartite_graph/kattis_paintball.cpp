...
void solve() 
{
    ...
    // about: Basic bipartite Assignment with maxflow (O(sqrt(n+n) * n))
    int n, m; cin >> n >> m;
    // structure: source, n persons, n persons, sink
    MaxFlow mf(n+n+2);
    int offset_targets = n+1;
    int sink = n+n+1;
    FOR(j, m){
        int a, b; cin >> a >> b;
        a--; b--;
        mf.add_edge(1+a, offset_targets+b, 1);
        mf.add_edge(1+b, offset_targets+a, 1);
    }
    FOR(i, n){
        mf.add_edge(0, 1+i, 1);
        mf.add_edge(offset_targets+i, sink, 1);
    }
    ...
    if(ans != n){
        ...
        FOR(i, n){
            ...
            int target = get<0>(v[0]);
            int targetPerson = target - offset_targets + 1;
            cout << targetPerson << endl;
        }
    }
