...
void solve() 
{
    // problem: https://open.kattis.com/problems/bookclub
    // about: Finding a perfect matching by checking if the MCBM is equal to n. As this graph is bipartite, the complexity is O(sqrt(V) * E), where V here is n*2 and E equals n.
    // Thus: O(sqrt(n*2) * n)
    int n, m; cin >> n >> m;
    // structure: source, n left nodes, n right nodes, sink
    MaxFlow mf (n*2 + 2);
    int offset_right = n+1;
    int sink = n*2+1;
    FOR(j, m){
        int a, b; cin >> a >> b; 
        mf.add_edge(a+1, offset_right + b, 1);
    }
    FOR(i, n){
        mf.add_edge(0, 1+i, 1);
        mf.add_edge(offset_right+i, sink, 1);
    }
