...
void solve() 
{
    // problem: https://open.kattis.com/problems/pianolessons
    // about: Maxflow assignment problem
    int n, slots;
    cin >> n >> slots;
    // structure: source, n students, slots, sink
    MaxFlow mf(n+slots+2);
    int offset_slots = n+1;
    int sink = n+slots+1;
    FOR(i, n){
        int t; cin >> t; 
        FOR(j, t){
            int s; cin >> s;
            s--;
            mf.add_edge(1+i, offset_slots+s, 1);
        }
        mf.add_edge(0, 1+i, 1);
    }
    FOR(i, slots){
        mf.add_edge(offset_slots+i, sink, 1);
    }
