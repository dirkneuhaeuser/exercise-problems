...
void solve() 
{
    // problem: https://open.kattis.com/problems/escapeplan
    // about: Basic MCBM problem. Here Dinic runs in O(sqrt(n+m) * n*m);
    int n; 
    int cnt = 1;
    while(cin >> n && n){
        ...
        for(int time: times){
            // structure: source, n robots, m holes, sink
            MaxFlow mf(n+m+2);
            ...
            for(int i=0; i<robots.size(); ++i){
                ...
                for(int j=0; j<holes.size(); ++j){
                    pdd h = holes[j];
                    ld d = dis(r, h);
                    if(d/10 + 0.0000001 <= time){
                        mf.add_edge(1+i, offset_holes+j, 1);
                    }
                }
            }
            ll ans = mf.dinic(0, sink);
            cout << "In " <<  time <<" seconds " << ans << " robot(s) can escape" << endl;
        }
        cout << endl;
    }
