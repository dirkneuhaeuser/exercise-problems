...
void solve() 
{
    // using dinics maxflow algorithm to solve MCBM problem
    int n, m; cin >> n >> m;
    MaxFlow maxFlow(2 + n + m);
    int sink = n+m+1;
    int offset_bolts = n+1;
    FOR(i, n){
        maxFlow.add_edge(0, i+1, 1);
    }
    FOR(j, m){
        maxFlow.add_edge(offset_bolts + j, sink, 1);
    }
    FOR(i, n){
        FOR(j, m){
            int c; cin >> c;
            if(c == 1){
                maxFlow.add_edge(1+i, offset_bolts+j, 1);
            }
        }
    }
    ll ret = maxFlow.dinic(0, sink);
