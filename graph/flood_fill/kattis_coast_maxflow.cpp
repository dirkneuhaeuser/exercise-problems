...
void solve() 
{
    // This problem could be models as Maxflow, but TLE :P
    int n, m; cin >> n >> m;
    n+=2;
    m+=2;
    ...
    MaxFlow maxFlow = MaxFlow(n*m+2);
    int source = n*m;
    int sink = n*m+1;
    FOR(i, n){
        int cellLeft = i*m;
        maxFlow.add_edge(cellLeft ,sink, INF);
        int cellRight = i*m + m-1;
        maxFlow.add_edge(cellRight ,sink, INF);
    }
    FOR(i, m){
        if(i == 0 || i == m-1) continue;
        int cellTop = i;
        maxFlow.add_edge(cellTop, sink, INF);
        int cellDown = (n-1) * m + i;
        maxFlow.add_edge(cellDown, sink, INF);
    }
    FOR(i, n){
        FOR(j, m){
            int cell = i*m + j;
            if(i < n-1){
                ...
                if(grid[i][j] == grid[i+1][j]){
                    maxFlow.add_edge(cell,cellN, INF, false);
                }else{
                    maxFlow.add_edge(cell,cellN, 1, false);
                }
            }
            if(j < m-1){
                int cellN = cell+1;
            ...
            if(grid[i][j] == 1){
                maxFlow.add_edge(source, cell, INF);
            }
        }
    }
    ll ret = maxFlow.dinic(source, sink);
    cout << ret;
