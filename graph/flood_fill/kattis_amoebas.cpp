...
void dfs(int iCur, int jCur, vector<vector<char>> &grid){
    ...
    // diagonal top right
    if(iCur>0 && jCur<m-1 && v[iCur-1][jCur+1] == -1 && grid[iCur-1][jCur+1] == '#'){
        dfs(iCur-1, jCur+1, grid);
    }

    // diagonal down right
    if(iCur<n-1 && jCur<m-1 && v[iCur+1][jCur+1] == -1 && grid[iCur+1][jCur+1] == '#'){
        dfs(iCur+1, jCur+1, grid);
    }

    // diagonal down left
    if(iCur<n-1 && jCur>0 && v[iCur+1][jCur-1] == -1 && grid[iCur+1][jCur-1] == '#'){
        dfs(iCur+1, jCur-1, grid);
    }
}
