...
void solve() 
{
    // Number of paths from s to t of Length L in an undirected Graph
    // matrix[i][j] number of ways to get from j to i
    // Use matrix to the power L
    ll ships, trips, start, end;
    while(cin >> ships >> trips){
        cin >> start >> end;
        start--; end--;
        Matrix trans(ships);
        FOR(i, ships){
            FOR(j, 4){
                int target;
                cin >> target;
                trans.mat[target-1][i]++;
            }
        }
        Matrix ret = modPowMat(trans, trips);
        cout << ret.mat[end][start] << endl;
