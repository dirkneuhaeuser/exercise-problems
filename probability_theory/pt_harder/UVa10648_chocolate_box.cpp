...
void solve() 
{
    int choco, boxes;
    int count = 1;
    while(cin >> choco && choco!= -1){
        cin >> boxes;
        vector<vector<ld>> dp (choco+1, vector<ld>(boxes+1, 0));
        dp[0][boxes] = 1;
        // dp[i][j] the probability that there are still j free boxes after the ith candy
        for(int i=0; i< choco; ++i){
            for(int j=1;j<=boxes;++j){
                dp[i+1][j] += (ld)(boxes-j)/(ld)boxes * dp[i][j]; 
                dp[i+1][j-1] += (ld)j/(ld)boxes * dp[i][j]; // candy in free box
            }
        }
        ld ret = 0;
        for(int j=1; j<=boxes; ++j){
