...
bigint powB(ll a, ll p){
    if(p==0) return 1;
    bigint ans = powB(a, p>>1);
    ans = ans*ans;
    if(p&1) ans *= a;
    return ans;
}
...
void solve() 
{
    ...
    while(cin>>F >> N >>S){
        int maxE = N*F;
        vector<vector<bigint>>dp(N+1, vector<bigint>(maxE+1, 0));
        // dp[i][j] denots the absolute possibilties to achieve a sum of j after the ith throw
        dp[0][0] = 1;
        for(int i=1; i<N+1; ++i){
            for(int j=1; j<maxE+1; ++j){
                for(int k=1; k<min(F+1, j+1); ++k){
                    dp[i][j] += dp[i-1][j-k];
                }
            }
        }
        if(S>maxE) cout << "0/" << powB(F,N) << endl;
        else cout << dp[N][S] << "/" << powB(F, N) << endl;
    }
