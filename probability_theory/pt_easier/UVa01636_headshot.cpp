...
void solve() 
{
    string s;
    while(cin >> s){
        int rotate = count(s.begin(), s.end(), '0');
        double rotateP = (double) rotate/ (double) s.size();
        ...
        double shootP = (double) shoot / ((double) shoot + (double) nshoot);
        if(shootP>rotateP) cout << "SHOOT" << endl;
        else if(shootP<rotateP) cout << "ROTATE" << endl;
