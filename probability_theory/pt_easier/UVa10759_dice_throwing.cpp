...
vector<vector<ull>> dp(25, vector<ull>(25*6 + 1,0));
...
{
    int n ,x;
    // idea: use dp[i][j] to calcuate the possibilties to achive a sum j after i throws;
    while(cin >> n >> x){
        if(n ==0 && x==0) return;
        ull nom = 0;
        for(int i=x; i<= 24*6; ++i){
            nom+= dp[n][i];
        }
        ull denom = powM(6, n);
        if(nom ==0){
            ...
            ull d = __gcd(nom, denom);
            cout << nom/d <<"/" << denom/d <<endl;
        }
    }
