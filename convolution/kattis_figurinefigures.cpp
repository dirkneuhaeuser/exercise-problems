...
void solve() 
{
    // with (f*f*f*f) we get the freqs of all possible 4-pack weights.. as it is only important to us whether this weight is possible ( frequency >=1) or not (frq ==0) and as numbers a quite big, we compute first (f*f) and recude all values greate than 1 to 1...this doesnt have an impact on the amount of weights possible!
    int N;
    cin >> N;
    vector<ll>f(60002, 0);
    int maxE = 3 << 30, minE = 1 << 30;
    ull mean = 0;
    FOR(i, N){
        int num; cin >> num;
        f[num]++;
    ...
    cout << fixed <<4* maxE << " " << 4* minE << " " << cnt << " " << (float)mean/(float)N*(float)4;
